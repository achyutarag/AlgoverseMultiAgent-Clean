...
# Clean baseline: import from local agents (no MCP bootstrap)
...
DEFAULT_CONFIG = {
    ...
    "retrieval": {
        "top_k": 5,
        "min_similarity": 0.2,  # ✅ Matches retriever default (same as MCP)
        "max_documents": 10
    },
    "extraction": {
        "max_documents": 3,
...
class MARAGPipeline:
    ...
    def __init__(
        ...
        **overrides
        ...
        # Create model_config dict for agents
        model_config_dict = {
        ...
        self.extractor = ExtractorAgent(
            ...
            max_tokens=self.config["extraction"].get("max_tokens", 2048)
        )
    ...
    async def _process_subquery_dynamic(
        ...
        try:
            ...
            retrieval_result = await self.retriever.process({
                "query": query_text,
                "k": 5,
                "min_similarity": self.config["retrieval"].get("min_similarity", 0.2)  # ✅ Use config value
            })
    ...
    def _aggregate_step_answer(self, sub_query_results: List[Dict[str, Any]], step_context: Dict[str, Any]) -> str:
        ...
        # Combine answers (clean, no confidence prefix - for StepDefinerAgent to use)
        answers = []
        for result in successful_results:
            answer = result.get("answer", "")
            if answer:
                answers.append(answer)  # ✅ Clean answer for step_answers dict
    ...
    async def _generate_final_answer_ma_rag(
        ...
        Uses simple synthesis logic instead of calling QA agent again to avoid "unknown" answers.
        
        Args:
            question: The original question
            plan: The execution plan
            steps_results: Results from each processing step
            max_context_length: Maximum context length to use for the final answer (unused in simple synthesis)
        ...
        for step in steps_results:
            ...
            if step_answer:
                # ✅ FIX: Extract clean answer text (remove [Confidence: X] prefix)
                # Remove confidence prefix if present
                clean_answer = re.sub(r'^\[Confidence:\s*[\d.]+\]\s*', '', step_answer).strip()
                ...
                step_answers.append({
                    ...
                    "answer": clean_answer,  # ✅ Use clean answer
        ...
        # ✅ FIX: Simple synthesis instead of calling QA agent again
        # For multi-hop questions, use the final step's answer
        query_type = plan.get('query_type', 'simple')
        
        if query_type == "multi-hop" and step_answers:
            # Multi-hop: extract the final step's answer
            final_step = step_answers[-1]
            final_answer = final_step["answer"]
            
            # If answer contains "unknown" but we have other steps, try to extract entity
            if "unknown" in final_answer.lower() and len(step_answers) > 1:
                # Look for entity names in previous steps
                for step_ans in reversed(step_answers[:-1]):
                    answer_text = step_ans["answer"]
                    # Extract potential entity (capitalized words)
                    entities = re.findall(r'\b([A-Z][a-z]+(?:\s+[A-Z][a-z]+)*)\b', answer_text)
                    if entities and len(entities[0]) > 2:
                        final_answer = entities[0]
                        break
            
            # Clean up the answer
            if final_answer.lower().strip() == "unknown":
                # Try to extract from all steps
                all_text = " ".join([sa["answer"] for sa in step_answers])
                entities = re.findall(r'\b([A-Z][a-z]+(?:\s+[A-Z][a-z]+)*)\b', all_text)
                if entities:
                    # Return the most common entity or the last one
                    final_answer = entities[-1] if entities else "unknown"
            
            return final_answer.strip()
        
        elif len(step_answers) == 1:
            # Single step: return the answer directly
            return step_answers[0]["answer"].strip()
        
        else:
            # Multiple steps: combine intelligently
            # Sort by confidence
            step_answers.sort(key=lambda x: x["confidence"], reverse=True)
            
            # For entity questions, extract the entity from the highest confidence answer
            query_lower = question.lower()
            if any(word in query_lower for word in ["who", "what", "which"]) and " or " not in query_lower:
                # Simple entity question - extract entity name
                best_answer = step_answers[0]["answer"]
                entities = re.findall(r'\b([A-Z][a-z]+(?:\s+[A-Z][a-z]+)*)\b', best_answer)
                if entities:
                    return entities[0]
                return best_answer.strip()
            
            # Otherwise, combine answers
            combined = "\n\n".join([f"Step {i+1}: {sa['answer']}" for i, sa in enumerate(step_answers[:3])])
